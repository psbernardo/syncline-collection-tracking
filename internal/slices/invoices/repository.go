...
func (r *GormRepository) CreateFromSalesOrder(ctx context.Context, orderID int64, number string, invoiceDate time.Time, key, requestID, actorID string) (Invoice, error) {
	...
	err = r.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		taxCodes := make([]string, 0, len(sourceLines))
		for _, line := range sourceLines {
			taxCodes = append(taxCodes, line.TaxCode)
		}
		receivableTaxRule, err := ReceivableTaxRule(taxCodes)
		if err != nil {
			return err
		}
		var duplicate int64
		...
		if err := tx.Raw(`INSERT INTO dbo.invoices
            (invoice_number, sales_order_id, sales_order_number, company_account_id,
             customer_name, billing_address, delivery_address, contact_person,
             contact_number, email, customer_po_number, sales_person, terms_days,
             invoice_date_utc, due_date_utc, subtotal_scaled, tax_scaled, total_scaled,
             status, created_at_utc, updated_at_utc)
            OUTPUT INSERTED.invoice_id
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, SYSUTCDATETIME(), SYSUTCDATETIME())`,
			invoice.Number, invoice.SalesOrderID, invoice.SalesOrderNumber, invoice.CompanyAccountID,
			invoice.CustomerName, invoice.BillingAddress, invoice.DeliveryAddress, invoice.ContactPerson,
			invoice.ContactNumber, invoice.Email, invoice.CustomerPONumber, invoice.SalesPerson,
			invoice.TermsDays, invoice.InvoiceDate, invoice.DueDate, invoice.Subtotal, invoice.Tax,
			invoice.Total, invoice.Status).Scan(&invoice.ID).Error; err != nil {
		...
		if invoice.ID == 0 {
			return errors.New("invoice identity was not generated")
		}
		for _, line := range sourceLines {
		...
		deliveryDate := invoiceDate.UTC()
		if deliveryDate.IsZero() {
			deliveryDate = time.Now().UTC()
		}
		gross := money.Amount(invoice.Total)
		breakdown, err := tax.CalculateRule(gross, receivableTaxRule)
		if err != nil {
			return err
		}
		businessDelivery, err := businessdate.Parse(businessdate.FormatUTC(deliveryDate))
		if err != nil {
			return err
		}
		dueDate, err := businessdate.DueDate(businessDelivery, order.Terms)
		if err != nil {
			return err
		}
		var receivableID int64
		if err := tx.Raw(`INSERT INTO dbo.delivery_receivables
            (company_account_id, invoice_id, invoice_number, po_number, po_number_normalized,
             delivery_date_utc, payment_term_days, due_date_utc, amount_due_scaled,
             gross_amount_scaled, tax_rule_code, ewt_amount_scaled, tax_base_scaled,
             vat_amount_scaled, lifecycle_status, created_at_utc, updated_at_utc)
            OUTPUT INSERTED.delivery_receivable_id
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'Active', SYSUTCDATETIME(), SYSUTCDATETIME())`,
			order.AccountID, invoice.ID, invoice.Number, order.PO, strings.ToUpper(strings.TrimSpace(order.PO)), businessDelivery,
			order.Terms, dueDate, breakdown.NetAmount.Int64(), breakdown.GrossAmount.Int64(), nullableTaxRule(receivableTaxRule),
			breakdown.WithholdingAmount.Int64(), breakdown.TaxBase.Int64(), breakdown.VATAmount.Int64()).Scan(&receivableID).Error; err != nil || receivableID == 0 {
			return errors.New("receivable identity was not generated")
		}
		updated := tx.Model(&struct{}{}).Table("dbo.sales_orders").Where("sales_order_id = ? AND status = 'OPEN'", orderID).Update("status", "CONVERTED")
		...
		receivableAfter, _ := json.Marshal(map[string]interface{}{"receivable_id": receivableID, "invoice_id": invoice.ID, "invoice_number": number, "company_account_id": order.AccountID, "po_number": order.PO, "gross_amount_scaled": breakdown.GrossAmount.Int64(), "tax_rule_code": nullableTaxRule(receivableTaxRule), "payment_term_days": order.Terms})
		if err := tx.Table("dbo.audit_events").Create(map[string]interface{}{"entity_type": "delivery_receivable", "entity_id": receivableID, "action": "create_from_invoice", "actor_id": actorID, "occurred_at_utc": time.Now().UTC(), "request_id": requestID, "idempotency_key": key, "previous_values_json": "null", "new_values_json": string(receivableAfter)}).Error; err != nil {
			return err
		}
...
func nullableTaxRule(rule tax.RuleCode) interface{} {
