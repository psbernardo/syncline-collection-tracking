...
| `invoice_number` | `VARCHAR(100)` | Yes | Optional trimmed ASCII alphanumeric value; NULL when no manual or linked invoice number is supplied |
...
- The filtered `UX_delivery_receivables_invoice_not_cancelled` index prevents duplicate non-cancelled invoice numbers when a value is supplied.
...
- `invoice_number` is optional after migration `0026`; existing invoice numbers are preserved and missing values remain NULL.
